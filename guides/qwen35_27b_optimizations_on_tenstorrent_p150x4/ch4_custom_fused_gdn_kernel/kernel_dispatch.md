...
The entry point is `gdn_full_fused_inplace()` (lines 609–656 of `gdn_kernel_op.py`), which accepts 11 tensors plus configuration parameters:
...
The function detects whether the full fused kernel is available via the `GDN_DISABLE_FULL_FUSED` environment variable. If the kernel fails at runtime, `_full_fused_available` is set to `False` globally and subsequent calls raise `RuntimeError`, signaling the caller to use the unfused path.

`z` (the SiLU gate projection) is **not** passed to the kernel. The SiLU gate is applied by the caller in Python after the kernel returns, via `ttnn.silu`. This design keeps the kernel focused on the recurrence-critical path and avoids adding another tensor input (see `gdn.py` lines 334–338).

## Program Construction: `_build_full_fused_device_program`

The core of the dispatch is `_build_full_fused_device_program()` (lines 325–498 of `gdn_kernel_op.py`), which builds a `ProgramDescriptor` for a single device. It receives the per-device tensor handles (already extracted from the mesh via `ttnn.get_device_tensors`) and constructs:
...
Pairs are distributed across cores using a simple division with remainder (lines 340–346):
...
Cores are assigned in column-major order across the compute grid: `CoreCoord(i % grid.x, i // grid.x)`. The first `remainder` cores each get one extra pair. For `num_pairs=384` and `num_cores=40`, this gives 9 pairs per core with 24 cores getting an extra pair (10 pairs each).

Cores with different pair counts are grouped separately for compile-time specialization — the `num_pairs` compile-time argument differs between groups, so each group gets its own set of `KernelDescriptor` objects (lines 423–493).

### Runtime Arguments

**Reader runtime args** (12 values per core, lines 361–374):
...
**Writer runtime args** (4 values per core, lines 375–380):
...
**Reader and Writer compile-time args** (lines 447–459):

| Index | Value | Kernels | Description |
|-------|-------|---------|-------------|
| 0 | `Kt = 4` | Both | Key dimension in tiles (`Dk / 32`) |
| 1 | `Vt = 4` | Both | Value dimension in tiles (`Dv / 32`) |
| 2 | `BF16_TILE_BYTES = 2048` | Both | Tile size in bytes |
| 3 | `state_l1_flag` | Both | 1 if state is in L1, 0 if DRAM |
| 4 | `packed_reduce_scaler = 0x3F803F80` | Reader | Packed bfloat16 1.0 for reduce operations |
| 4 | 0 | Writer | Unused (kept for compatibility) |
| 5 | `Nv_TP = 12` | Reader | Value heads per device |
| 5 | 0 | Writer | Unused (kept for compatibility) |
| 6 | `Nk_TP = 4` | Reader | Key heads per device |
| 6 | `sharded_flag` | Writer | 1 if state is HEIGHT_SHARDED |
| 7 | `repeat_factor = 3` | Reader | Value heads per key head (`Nv_TP / Nk_TP`) |
| 8 | `key_tile_off = 16` | Reader | Tile offset to K region in `conv_out` (`key_dim_tp / 32`) |
| 9 | `v_tile_off = 32` | Reader | Tile offset to V region in `conv_out` (`2 * key_dim_tp / 32`) |
| 10 | `sharded_flag` | Reader | 1 if state is HEIGHT_SHARDED in L1, 0 otherwise |

**Compute compile-time args** (3 values, line 483):
...
The compute kernel also receives a `ComputeConfigDescriptor` (lines 485–490) specifying `MathFidelity.HiFi4`, `fp32_dest_acc_en=True`, and `math_approx_mode=False` for maximum numerical precision during the recurrence.

## Circular Buffer Descriptors

The program defines 28 circular buffers (lines 388–418). Each is created via the `_make_cb` helper with a CB index, tile count, and the core range set. All use bfloat16 format with 2048-byte pages.
...
| `c_15` | `cb_scale` | 1 | Q scale $D_k^{-0.5}$ (reader fills once, persistent) |
...
| `c_20` | `cb_rms_eps` | 1 | $D_v \cdot \varepsilon$ for RMS norm stability (persistent) |
| `c_21` | `cb_scratch` | 1 | Reader scratch buffer for sub-tile extraction |
| `c_22` | *(unused/reserved)* | — | Not allocated; index gap between `c_21` and `c_24` |
| `c_23` | *(unused/reserved)* | — | Not allocated; index gap between `c_21` and `c_24` |
...
Total L1 per core: $(4+4+4+4+1+1+16+16+16+1+1+1+1+4+1+4+4+4+1+1+1+1+4+4+4+4+1+1) \times 2048 = 109 \text{ tiles} \times 2\text{ KB} = 218\text{ KB}$. This is well within the ~1.2 MB L1 budget per core on Blackhole.

`c_11` (originally `cb_z`) and `c_30` (originally `cb_rec_out`) were removed during development. The SiLU gate is handled in Python post-kernel, and the recurrence output is written directly to `cb_out` rather than through an intermediate buffer (line 414 comment).

## Multi-Device Dispatch

For the P150x4 with TP=4, each device processes its own shard of the data. The dispatch path (lines 550–568) is:
...
The critical detail is that each device's program gets its own buffer addresses via `devs[i].buffer_address()`. Even though the program structure is identical across devices, the runtime args differ because each device's tensors reside at different physical addresses.

## Program Cache and Kernel Hashing

The module computes an MD5 hash of all kernel source files at import time via `_compute_kernel_hash()` (lines 52–64):

```python
def _compute_kernel_hash():
    h = hashlib.md5()
    for path in [
        READER_PATH,        # dataflow/reader_gdn.cpp
        WRITER_PATH,        # dataflow/writer_gdn.cpp
        READER_IAF_PATH,    # dataflow/reader_gdn_iaf.cpp
        WRITER_IAF_PATH,    # dataflow/writer_gdn_iaf.cpp
        COMPUTE_PATH,       # compute/gdn_recurrence.cpp
        READER_FUSED_PATH,  # dataflow/reader_gdn_fused.cpp
        WRITER_FUSED_PATH,  # dataflow/writer_gdn_fused.cpp
        COMPUTE_FUSED_PATH, # compute/gdn_fused.cpp
    ]:
...
**Next:** [`reader_kernel.md`](./reader_kernel.md)
