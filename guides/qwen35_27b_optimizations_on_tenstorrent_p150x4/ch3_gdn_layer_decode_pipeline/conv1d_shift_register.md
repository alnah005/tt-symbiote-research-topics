...
The conv1d state consists of 4 tensors stored in `self.conv_states`, a Python list (`gdn.py` lines 175–178):
...
Each decode step shifts the register by copying each slot to its predecessor, then writes the new input into the last slot (`gdn.py` lines 281–284):
...
After shifting, the convolution output is computed as a weighted sum of all 4 slots using the learned tap weights (`gdn.py` lines 286–292):
...
The `tw["conv_taps"]` list contains 4 weight tensors, each of shape `[1, 1, qkv_dim_tp]` = `[1, 1, 2560]`, broadcast across the batch dimension. These weights are prepared during model loading by the `prepare_conv_taps` function in `model_config.py` (lines 375–393), which extracts per-TP shards of the learned convolution kernel interleaved in Q/K/V head order.
...
Two reset methods maintain trace compatibility (`gdn.py` lines 164–208):
...
Each conv state slot occupies `1 * 32 * 2560 * 2 = 163,840 bytes ≈ 160 KB` in bfloat16 (before tile padding). With 4 slots, the total conv state per layer is approximately `640 KB`. Across all 48 GDN layers, this is approximately `30 MB` per device — a small fraction of the recurrence state memory (which is approximately `576 MB` per device for 48 layers at 12 MB each).

---

**Next:** [`recurrence_math.md`](./recurrence_math.md)
