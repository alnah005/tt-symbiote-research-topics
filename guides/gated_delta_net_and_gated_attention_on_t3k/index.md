...
- 30 **Gated Delta Net** layers — linear-recurrent attention with a fixed-size state matrix `S ∈ R^{d_k × d_v}` updated via the delta rule with a scalar decay gate
...
Gated Delta Net combines GLA-style scalar decay (`g_t = exp(α_t) ∈ (0,1)`) with DeltaNet-style error-correcting writes. Master recurrence: `S_t = g_t · S_{t-1} + k̃_t (β_t · (v_t − g_t · S_{t-1}^T k̃_t))^T`. See Ch1 (taxonomy) and Ch2 (derivation).

**Q2 — What is Gated Attention?**
Qwen3.5-specific SDPA: Q is projected with a sigmoid gate (Q_gated = σ(gate) ⊙ Q), then Q and K are normalized per-head (RMSNorm), then RoPE applied to 64 of 256 head dimensions, then standard SDPA with 2 KV heads expanded 8× to serve 16 Q heads. See Ch3.
