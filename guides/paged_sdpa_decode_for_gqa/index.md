...
| Ch | Directory | Description | Key files |
|----|-----------|-------------|-----------|
| 1 | `ch1_gqa_paged_fundamentals/` | Establishes vocabulary: GQA head-count invariant (`nh = nkv * group_size`), paged KV block model, `page_table_tensor` semantics, and why GQA expansion must never be applied before writing to a paged cache. | `gqa_concept.md`, `paged_kv_cache_concept.md`, `gqa_plus_paging_interaction.md` |
| 2 | `ch2_ttnn_api/` | Documents every parameter of `ttnn.transformer.scaled_dot_product_attention_decode`: tensor shapes for paged and non-paged modes, layout and dtype constraints, `SDPAProgramConfig` fields, and the `num_cores >= b * nkv` grid-size constraint. | `function_signature.md`, `tensor_shape_reference.md`, `sdpa_program_config.md` |
| 3 | `ch3_gqa_tensor_layout/` | Specifies exactly how TTNN expects Q, K, V in memory for GQA decode, including the 32-padding silent bug that collapses GQA to MQA when padding changes the Q/KV head ratio, and the old vs. current dense KV cache axis ordering. | `head_axis_conventions.md`, `gqa_grouping_in_kernel.md`, `paged_layout_for_gqa.md` |
| 4 | `ch4_cur_pos_semantics/` | Defines `cur_pos[i]` precisely (pre-write current context length; equals the 0-indexed position of the next token to be written), covers the Python-list vs. device-tensor trade-off, documents three common off-by-one and shape mistakes, and explains how `cur_pos` drives both causal masking and physical block selection in paged mode. | `cur_pos_definition.md`, `per_user_vs_shared.md`, `cur_pos_in_paged_mode.md` |
| 5 | `ch5_known_issues/` | Catalogs all documented bugs and silent failure modes: Issue #30362 (sporadic PCC failures at block boundaries, open as of early 2026), four silent shape violations, the `repeat_interleave` / native GQA mixing bug, and program cache issues #21534 and #16674. | `issue_30362_pcc_failures.md`, `silent_shape_violations.md`, `gqa_workaround_history.md`, `program_cache_issues.md` |
| 6 | `ch6_debugging/` | Provides a four-step debug ladder (shape audit → `cur_pos` validation → PCC comparison → root-cause isolation) with a PyTorch reference implementation, PCC thresholds, binary search strategy for Issue #30362 boundary failures, and an escalation flowchart. | `shape_validation_checklist.md`, `cur_pos_validation.md`, `pcc_comparison_workflow.md`, `root_cause_isolation.md` |
