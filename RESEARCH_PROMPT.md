...
16. **All mathematical equations must use LaTeX formatting.** Agent A is responsible for applying this when writing content:
    - **Display (block) equations** use `$$...$$` on their own lines, **or ` ```math ` fenced blocks** (see constraints below).
    ...
    - Agent B must flag any display equation written in plain text or a code block (rather than `$$...$$` or ` ```math `) as a structural gap.
    - **Never use underscores inside `\text{...}`** — the markdown parser consumes the `\` in `\_` before the math renderer sees it, producing a bare `_` that causes an "allowed only in math mode" error. Use spaces instead: `\text{expert capacity}` not `\text{expert\_capacity}`.
    - **Never use `\texttt{name_with_underscores}` in `$$...$$` or `$...$` blocks.** In text mode (`\texttt`), bare `_` triggers "allowed only in math mode"; `\_` is consumed by the markdown parser; and `\textunderscore` is math-mode only and will fail in text mode. For **display equations** containing `\texttt` with underscores, use a ` ```math ` fenced block instead (GitHub does not preprocess fenced block contents, so `\_` works). For **inline expressions**, avoid `\texttt` with underscored names entirely — use math-mode subscript notation (`$A_{\log}$`) or move the code name to surrounding prose as a backtick identifier.
    - **Never use `\!` (negative thin space) in `$$...$$` blocks.** GitHub's markdown parser strips the backslash, leaving a literal `!` character in the rendered output. If you need `\!` for spacing, use a ` ```math ` fenced block instead, where the content is not preprocessed.
    - **Summary of when to use ` ```math ` fenced blocks vs `$$...$$`:** Use ` ```math ` when the equation contains (a) `\texttt` with underscores or (b) `\!`. Use `$$...$$` for all other display equations.
