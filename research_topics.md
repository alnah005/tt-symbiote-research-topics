...
## Wormhole N150 Peak FLOPS and Effective DRAM Bandwidth
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/wormhole_n150_peak_flops_and_effective_dram_bandwidth/`
**Why Needed:** Theoretical performance estimates for all models on T3K (roofline model, arithmetic intensity ridge point, compute-bound vs memory-bound classification) depend critically on two hardware constants: peak BF16/FP8 matmul FLOPS and effective DRAM bandwidth. The research cache currently marks both figures as unverified best-effort estimates (~262 TFLOPS BF16, ~300 GB/s DRAM BW per chip).
**Questions:**
- What is the exact peak BF16 matrix multiply throughput for a Wormhole N150 (B0) chip in TFLOPS (measured, not estimated)?
- What is the peak FP8 (bfloat8_b) matmul throughput — is it exactly 2× BF16 or a different ratio?
- What is the effective DRAM bandwidth under realistic access patterns (large interleaved tensors, decode workloads)?
- What is the intra-chip NOC peak bandwidth (L1-to-DRAM and L1-to-L1 transfers)?
- What is the arithmetic intensity ridge point (FLOPs/byte) that separates compute-bound from memory-bound regimes on Wormhole?

---

## Gemma 4 Small Variant Config Specs (1B, 4B, 12B, 27B)
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/gemma_4_small_variant_config_specs/`
**Why Needed:** The Gemma 4 family includes multiple dense text variants besides the 31B. Theoretical performance analysis and T3K deployment planning for smaller models requires exact config parameters from released HuggingFace checkpoints. Current estimates are extrapolated from the `Gemma4TextConfig` defaults and may be inaccurate.
**Questions:**
- What are the exact `config.json` parameters for each released Gemma 4 text variant (1B, 4B, 12B, 27B)? Specifically: `hidden_size`, `intermediate_size`, `num_hidden_layers`, `num_attention_heads`, `num_key_value_heads`, `num_global_key_value_heads`, `head_dim`, `global_head_dim`, `sliding_window`, `hidden_size_per_layer_input`.
- Do any of the smaller variants enable the MoE block (`enable_moe_block=true`)?
- What are the actual parameter counts for each variant?
- Which variants have PLE enabled (`hidden_size_per_layer_input > 0`)?

---

## Gemma 4 MoE Variant Architecture Details
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/gemma_4_moe_variant_architecture_details/`
**Why Needed:** The Gemma 4 HuggingFace config supports a parallel MoE + dense MLP architecture (`enable_moe_block=true`) that differs from all other MoE models currently in tt-symbiote (Qwen3MoE, Ling). Theoretical performance and T3K sharding strategy for MoE Gemma 4 variants requires knowing the exact expert count, routing parameters, and how the parallel MoE path interacts with the dense MLP path.
**Questions:**
- Which released Gemma 4 checkpoint(s) enable `enable_moe_block=true`? What are the exact model IDs on HuggingFace?
- What are the exact MoE parameters: `num_experts`, `top_k_experts`, `moe_intermediate_size`?
- How does the parallel MoE + dense MLP architecture work — do both paths run on the same residual and their outputs are summed, or is there a gating mechanism that selects one path?
- How should the expert weights be distributed across the 8 T3K devices — can experts be assigned EP=8 (1 expert group per device), or does the small top_k make a different assignment better?
- What is the active FLOPs vs. total parameter ratio for the MoE layers (top_k / num_experts × moe_intermediate_size)?

---

## Wormhole BFP8 and BFP4 Dense Matmul Throughput and Accuracy
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/wormhole_bfp8_bfp4_dense_matmul_throughput_and_accuracy/`
**Why Needed:** Decode throughput estimates for all dense transformer models on T3K assume that bfloat8_b (BFP8) weights deliver 2× the bandwidth-bound throughput of bfloat16, and bfloat4_b (BFP4) delivers 4×. Real hardware may have non-linear scaling due to pack/unpack overhead, tile format constraints, or core utilization differences. Accuracy data for dense (non-MoE) models at BFP8/BFP4 is also needed.
**Questions:**
- What is the measured decode throughput improvement for `bfloat8_b` vs `bfloat16` weights for matrix-vector multiplies (batch=1) on Wormhole N150? Is it close to the theoretical 2× from halved bytes?
- What is the measured throughput for `bfloat4_b` vs `bfloat8_b`? Is it close to 2× again, or does pack/unpack overhead reduce gains?
- What accuracy degradation has been measured for dense transformer models (LLaMA/Gemma class) at BFP8 and BFP4 weight precision on standard benchmarks?
- Does Gemma 4's V-norm (RMSNorm applied to value vectors before KV cache write) help stabilize BFP8 KV cache quality compared to models without V-norm?
- Is BFP8 KV cache viable for production deployment on Gemma-class models without perceptible quality loss?

---

## T3K Measured Decode Throughput for Reference Models (Calibration Data)
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/t3k_measured_decode_throughput_reference_models/`
**Why Needed:** Theoretical bandwidth-bound decode throughput estimates are validated only when calibrated against measured performance on real hardware. Reference data from similar-sized models already running on T3K would provide confidence intervals for new model estimates and reveal the typical gap between theoretical and achieved throughput.
**Questions:**
- What is the measured decode throughput on T3K for LLaMA 3 70B (BFP8, B=1, S=8K) at baseline and with the full optimization stack (Metal Trace + DRAM-sharded)?
- What is the measured decode throughput for Qwen2.5 32B on T3K?
- What is the typical ratio of measured throughput to theoretical bandwidth-bound estimate for the best-optimized models?
- What is the measured prefill throughput (tokens/sec) for LLaMA 3 70B or similar at S=2K and S=8K?
- How does the CCL overhead (all-reduce latency per step) vary with the actual tensor sizes and ring topology on T3K?

---

## Gemma 4 Per-Layer Embedding (PLE) Performance Impact on T3K
**Date:** 2026-04-15
**Status:** Pending
**Guide:** `guides/gemma_4_per_layer_embedding_performance_impact/`
**Why Needed:** Gemma 4 smaller variants (1B, 4B config) use Per-Layer Embeddings (PLE) controlled by `hidden_size_per_layer_input=256`. This mechanism adds a large secondary embedding table lookup and per-layer projection that is not present in standard transformer analysis. For a 2B model, the secondary embedding table alone may exceed the weight size of the entire model, and the per-layer conditioning adds latency at every decoder layer.
**Questions:**
- For variants with `hidden_size_per_layer_input=256`, what is the exact PLE mechanism — what does the secondary embedding table look like (`[vocab_size, num_layers × 256]`?), how is it indexed, and where does its output enter the layer computation?
- What is the memory footprint of the PLE secondary embedding table for each variant that uses it? Is it weight-shared across layers or stored per-layer?
- What is the per-layer latency cost of PLE injection during decode on T3K (embedding lookup + projection + norm + gated residual)?
- How should the PLE secondary embedding table be sharded across 8 T3K devices — column-parallel along the hidden dimension, or replicated?
- Which specific released Gemma 4 model sizes have PLE enabled, and is it expected to be a significant fraction of total decode latency?
